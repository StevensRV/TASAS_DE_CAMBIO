...
n_steps_in, n_steps_out = 1,1 # Número de filas con características que se ingresan al modelo y número de predicciones
...
# %%
class LSTM_Model:
    def __init__(self, n_steps_in=0, num_lags=0, seed=42, lr_scheduler=None):
        ...
        if lr_scheduler is None:
            self.lr_scheduler = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=5, min_lr=1e-5)
        else:
    ...
    def fit(self, X, y, epochs=20, batch_size=128, **kwargs):
...
def predict_n_steps_ahead(pipeline, initial_data, n_steps=None):
    ...
    if isinstance(model, (RandomForestRegressor, LinearRegression)):
        ...
        for _ in range(n_steps):
            # Reformatear last_data para que tenga la forma (1, 1, 6)
            input_data = last_data.reshape(1, 1, -1)  # Mantenemos las 6 características
            
            # Realizamos la predicción
            pred = pipeline.named_steps['model'].predict(input_data)
            rolling_predictions.append(pred[0])  # Almacenamos la predicción
            
            # Actualizamos los datos para la siguiente predicción
            last_data = np.roll(last_data, shift=-1, axis=0)  # Desplazamos las filas hacia arriba
            last_data[0, -2] = pred[0]  # Sustituimos el primer valor con la nueva predicción
            
            # Calcular el rendimiento respecto al paso anterior
            if len(rolling_predictions) > 1:
                rendimiento = (pred[0] - rolling_predictions[-2]) / abs(rolling_predictions[-2])
                last_data[0, -1] = rendimiento  # Actualizamos el último valor con el nuevo rendimiento
